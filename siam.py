...
df_screen_time_data = pd.read_csv('dataset2 (1).csv')
df_well_being_data= pd.read_csv('dataset3 (1).csv')



# Merge Dataset 2 and Dataset 3 on 'ID'
merged_data = pd.merge(df_screen_time_data, df_well_being_data, on='ID')
...
for var in well_being_indicators:
    ...
    if p_value < 0.05:
        conclusion = f"Reducing screen time improves {var}"
